...
for wavind = 1:wavnm
    ...
    s = filter(b,a,s);
    S = tdft(s,winlen,winadv);
    clear s
    
    S = log(max(S(trainstrct.freqinds,:),amp_cutoff)) - log(amp_cutoff);
   
    len = size(S,2);
    
    if pcaopt
        X = S - repmat(specmn',1,len);
        X = coeff'*X;
    else
        X = S;
    end
    
    Y = zeros(protonm+trainstrct.kopt,len)+inf;
    
    for sylind = 1:protonm
        
        spectmp = trainstrct.specarr{sylinds(sylind)};
        
        if size(X,2)>size(spectmp,2)
...
%     inds = 1:length(c);
    ...
    %     [tempons,tempoffs,matchids] =
    %     template2clips2(dt*inds',l(inds)'*dt,v(inds)');
    %     inds = inds(matchids);
