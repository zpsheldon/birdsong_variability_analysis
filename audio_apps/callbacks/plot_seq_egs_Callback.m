...
buff = 200;
buff = 5;
specbuff = round(buff / (winadv * 1000/ fs));
...
for i = inds
    ...
    elseif labopt
        ...
        onsall = tmstmp(oninds(i):oninds(i)+sylnm-1) - t0;
        lensall = lenstmp(oninds(i):oninds(i)+sylnm-1);
        labpos = onsall + lensall/2;
        
        for labind = 1:sylnm
            text(labpos(labind),midfreq,seq{labind},'fontsize',20,'color','k','HorizontalAlignment','center','fontweight','bold','VerticalAlignment','bottom')
        end
...
if labopt
    set(htmp,'Position',[0.2601    0.2705    0.4690    0.2010])
    set(gca,'Position',[.13 .12 .775 .7],'box','off','fontsize',16,'xtick',[],'ytick',[])
%     xlabel('time (msec)','fontsize',18','fontweight','bold')
%     ylabel('freq. (Hz)','fontsize',18','fontweight','bold')
end
