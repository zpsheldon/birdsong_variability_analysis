...
% falsepos = clipstrct.falseposvc ./ [protonm{:}];
% falseneg = clipstrct.falsenegvc ./ (clipnm-[protonm{:}]);

falsepos = clipstrct.falseposvc ./ (clipnm-[protonm{:}]);
falseneg = clipstrct.falsenegvc ./ [protonm{:}];
...
for dimind1 = 1:dimnm
    for dimind2 = 1:dimnm
        ...
        if dimind1 ~= dimind2           
            ...
            plot(h(dimind2,dimind1),[-1 threshvc-.5],[threshvc-.5 threshvc-.5],'k-','linewidth',2)
            plot(h(dimind2,dimind1),[threshvc-.5 threshvc-.5],[-1 threshvc-.5],'k-','linewidth',2)
...
%             ylim([0 1])
        ...
        if dimind2 == 1
            title(['false pos / neg = ' num2str(round(1000*falsepos(template_selectinds(dimind1)))/10) ' / ' num2str(round(1000*falseneg(template_selectinds(dimind1)))/10) '%'])
        end
