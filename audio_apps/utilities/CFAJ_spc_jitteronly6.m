function [psi,sigma,iter,n_fail,logp,psi0,sigma0] = CFAJ_spc_jitteronly6(X,bootN)
...
while i < bootN
    ...
    sigma0tmp = diag([vr(1:d-1)+vr(2:d)].*rand(d-1,1));
    psi0tmp = diag(vr.*rand(d,1));

    [sigmatmp,psitmp,itertmp,degflg] = CFA_jitteronly6(X,sigma0tmp,psi0tmp);
