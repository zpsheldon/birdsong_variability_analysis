function [W,psi,phi,sigma,omega,iter,n_fail,logp,Chat,W0,psi0,phi0,sigma0,omega0] = CFAfull_prior_spc_waitbar(X,Q,initN,m,D,R)
...
while i < initN
    ...
    [Wtmp,sigmatmp,psitmp,phitmp,omegatmp,itertmp,degflg] = CFA_full_priors(X,Q,m,W0tmp,sigma0tmp,psi0tmp,phi0tmp,omega0tmp,D,R,Wexp,.1,1);
    ...
    if itertmp < 100000 && degflg
        ...
        logp(i) = -n/2*(d*log(2*pi) + log(det(Chat)) + trace(inv(Chat)*C))-.5*sum((Wtmp(:)-Wexp(:)).^2)-sum(2*log(psitmp)+psitmp.^-2);
...
for i = 1:length(inds)
    ...
    [Wtmp,sigmatmp,psitmp,phitmp,omegatmp,itertmp,degflg,z,u] = CFA_full_priors(X,Q,m,W0tmp,sigma0tmp,psi0tmp,phi0tmp,omega0tmp,D,R,Wexp,0.1);
    
    Chatmp = Wtmp*Wtmp' + D*diag(sigmatmp)*D' + Q*diag(phitmp)*Q' + diag(psitmp) + R*diag(omegatmp)*R';
    logptmp = -n/2*(d*log(2*pi) + log(det(Chatmp)) + trace(inv(Chatmp)*C))-sum((Wtmp(:)-Wexp(:)).^2)-.5*sum(2*log(psitmp)+psitmp.^-2);
