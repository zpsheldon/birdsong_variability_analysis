function [seqmat,N] = vc2seqs(vc,N_min,M_min,maxrepeat,silind,vc0)
...
while contflg && M <= 15
    ...
    if ~isempty(indsval)
        ...
        indsvalval = zeros(1,seqnmtmp);
        m = 1;
        for i = 1:seqnmtmp          
            seqbintmp = seq2bin(seqmatmp(i,:));
            seqcorrtmp = xcorr212(seqbintmp,seqbintmp,M-1);
            maxrptlen = max(seqcorrtmp(1:M-1));
            
            if ~any(seqmatmp(i,2:end-1)==silind) &  maxrptlen <= maxrepeat
                indsvalval(m) = i;
                m = m + 1;
            end
        end
